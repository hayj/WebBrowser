...
class HTTPBrowser():
    ...
    def privateGet(self, crawlingElement, forcedPort=None, noProxy=False, isARetry=False, useTor=False, **kwargs):
        ...
        response = None
        try:
            if useTor or (not noProxy and self.hasProxy()):
                # if useTor and isHostname("tipi"):
                #     try:
                #         r = requests.get \
                #         (
                #             "https://api.ipify.org?format=jsonurl",
                #             proxies= \
                #             {
                #                 "http": proxyStr,
                #                 "https": proxyStr,
                #             }
                #         )
                #         log("eeeeeeeeeeeeeeeeeeeeeee TODO delete this", self)
                #         log(r.content, self)
                #     except:
                #         logException(e, self)
                # ttt = None
